...
onMounted(() => {
    store.getFreeList(store.selectedParty.partyId, "reg_date", "DESC",2);
})
...
const setOld = function () {
    store.getFreeList(store.selectedParty.partyId, "reg_date", "ASC",2);
}

const goArticleCreate = function () {
    router.push({ name: 'articleCreate', params: { category: "공지사항" } })
}
const searchQuery = ref("");
const performSearch = () =>{
    // console.log('dd')
    store.getFreeList(store.selectedParty.partyId, "reg_date", "DESC",2,'title',searchQuery.value)
}
</script>
