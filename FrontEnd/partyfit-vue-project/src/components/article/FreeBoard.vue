<template>
    <div>
      ...
      <div class="container">
        <table class="table table-hover text-center">
          ...
          <tbody>
            <tr v-for="article in currentPageArticleList" :key="article.articleId" @click="goArticleDetail(article.articleId)">
  ...
  const currentPageArticleList = computed(() => {
    return store.freeList.slice((currentPage.value - 1) * perPage, currentPage.value * perPage);
  });
  
  const goArticleDetail = function (articleId) {
    console.log(articleId);
    router.push({ path: '/partyfit/article/2', params: { articleId: articleId } });
  };
  ...
  const goArticleCreate = function () {
    router.push({ name: 'articleCreate', params: { category: '자유게시판', partyId: store.selectedParty.partyId } });
  };
  </script>
